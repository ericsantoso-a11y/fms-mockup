...
export default function ViewPickupGroupPage() {
  ...
  return (
    <FMSLayout breadcrumbs={[{ label: "Workforce Management" }, { label: "Pickup Group", href: "/pickup-group/list" }, { label: group.name }]}>
      <div className="max-w-2xl">
        <div className="bg-white rounded border border-gray-200 p-8 space-y-5">
          ...
          <Field label="Group Members" value={group.groupMembers.length === 0 ? "-" : group.groupMembers.map((m) => `[${m.id}] ${m.name}`).join(", ")} />

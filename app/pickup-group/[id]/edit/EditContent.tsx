...
export default function EditPickupGroupPage() {
  ...
  const handleSubmit = () => {
    if (!validate()) return;
    setSubmitting(true);
    setTimeout(() => { router.push(`/pickup-group/${id}/view`); }, 600);
  };

  return (
    <FMSLayout breadcrumbs={[{ label: "Workforce Management" }, { label: "Pickup Group", href: "/pickup-group/list" }, { label: `Edit: ${group.name}` }]}>
      <div className="max-w-2xl">
        <div className="bg-white rounded border border-gray-200 p-8 space-y-6">
          <div className="flex items-start gap-4">
            <label className="w-44 text-sm text-gray-700 pt-2 text-right flex-shrink-0"><span className="text-red-500 mr-1">*</span>Pickup Group Name</label>
          ...
          <div className="flex items-start gap-4">
            <label className="w-44 text-sm text-gray-700 pt-2 text-right flex-shrink-0"><span className="text-red-500 mr-1">*</span>Group Lead<span className="ml-1 text-gray-400 cursor-help" title="The main driver">ⓘ</span></label>
            <div className="flex-1">
              <select value={leadId} onChange={(e) => setLeadId(e.target.value)}
                ...
                {mockDriverOptions.map((d) => <option key={d.id} value={d.id}>[{d.id}] {d.name}</option>)}
              </select>
              {errors.lead && <p className="text-red-500 text-xs mt-1">{errors.lead}</p>}
            </div>
          </div>
          <div className="flex items-start gap-4">
            <label className="w-44 text-sm text-gray-700 pt-2 text-right flex-shrink-0">Group Member<span className="ml-1 text-gray-400 cursor-help" title="Select one or more sub drivers">ⓘ</span></label>
            <div className="flex-1">
              <MultiSelect options={mockDriverOptions} selected={memberIds} onChange={setMemberIds} placeholder="Please Select Sub Drivers" excludeIds={leadId ? [Number(leadId)] : []} />
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="relative">
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} maxLength={500} placeholder="Please Input" rows={4}
        ...
        <div className="flex justify-end mt-4 gap-3">
          ...
          <button onClick={handleSubmit} disabled={submitting} className="bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white px-6 py-2 rounded text-sm font-medium transition-colors">{submitting ? "Saving..." : "Save"}</button>
        </div>
      </div>
    </FMSLayout>
  );

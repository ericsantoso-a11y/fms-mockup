...
export default function ViewFMGroupPage() {
  const { id } = useParams();
    ...
    >
      <div className="max-w-2xl">
        <div className="bg-white rounded border border-gray-200 p-8 space-y-5">
          <Field label="FM Group ID" value={String(group.id)} />
          <Field label="FM Group Name" value={group.name} />
          <Field
            label="Group Lead"
            value={`[${group.groupLeadId}] ${group.groupLead}`}
          />
          <Field
            label="Group Members"
            value={
              group.groupMembers.length === 0
                ? "-"
                : group.groupMembers.map((m) => `[${m.id}] ${m.name}`).join(", ")
            }
          />
          <Field label="Description" value={group.description} />
          <Field
            label="Number of Drivers"
            value={String(group.groupMembers.length + 1)}
          />
        </div>
